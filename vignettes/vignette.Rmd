...
It is possible to add custom scales to the factors:

```{r}
fp_design_matrix(2) %>% 
  fp_add_scale(A=c(1, 10), B=c(75, 125), suffix=".scaled")
```
...
Now we can fit a model with the quadratic term, using either $A$ or $B$: since we only have a central point, we cannot discriminate which factor is contributing to the curvature in the response suface. We get:
